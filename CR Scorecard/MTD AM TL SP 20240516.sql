PROCEDURE CR_SC_MTD_AM_TL (
    per in VARCHAR2,vfusionid7 in VARCHAR2, p_result OUT SYS_REFCURSOR) 
    is 
    period date;
  BEGIN
    -- TODO: Implementation required for PROCEDURE CR_SC_MTD_AGNTS.CR_SC_MTD_AGNTS
      ...
      --vfusionid:='196035';
         ...
         if(vfusionid7 is null and period is null ) then
            OPEN p_result FOR  
  select 
...
left join(
select month,fusionid,ucid,ntid,livevox_id,name,team_leader,location,dept,designation,tl_fusionid from asit_roster_table
where month=last_day(month)
and dept in ('CCC-R','CCC-R SPANISH','FLEX')
and location in ('MUM','BAN','MTL','PMC','RC','STX','WPB')
and designation in ('Team Lead')
and final_status='ACTIVE'
and team_leader not in (select name from not_teamleader)
) b on a."MONTH"=b.month  and a."FUSION_ID"=b.fusionid
--where b.tl_fusionid='106072'
--where 
...
where 
a.month = last_day(period) and 
b.tl_fusionid in (vfusionid7)
--where 
--month>=last_day(period)
--and  
--a."FUSION_ID" in (vfusionid3)

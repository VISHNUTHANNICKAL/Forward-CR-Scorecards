...
/*
select month,fusionid,ucid,livevox_id,name,team_leader,location,dept,designation,final_status 
from asit_roster_table
where month=last_day(month) and
 dept in ('CCC-R','CCC-R SPANISH','FLEX')
and location in ('MUM','BAN','MTL','PMC','RC','STX','WPB')
and designation in ('Assistant Manager')
and final_status in('ACTIVE')
and team_leader not in (select name from not_teamleader) 
group by fusionid,ucid,livevox_id,name,team_leader,location,dept,designation,final_status

select * from cr_sc_am_target
*/
...
where a."MONTH" >= last_day('01-FEB-24')

...
coalesce(round(avg(a."Quality Score"),2),0) "Quality Score"
from (
select last_day(a.month) month,
a.fusionid,c.ucid,a.employee_name,c.tl_fusionid,a.supervisor_name
,c.dept
,coalesce(round(avg(a.qa_yield),2),0) "Quality Score"
,rank() OVER (PARTITION by last_day(a.month),c.dept ORDER BY coalesce(round(avg(a.qa_yield),2),0) desc,c.dept,last_day(a.month) )  "Quality Rank"
,b."Out Of"
,case 
when coalesce(round(avg(a.qa_yield),2),0) >=97.50  then 3.75 
when coalesce(round(avg(a.qa_yield),2),0) >=95.20  then 2.5
when coalesce(round(avg(a.qa_yield),2),0) >=90  then 1.25 
else 0
end as
"Quality_Score"
from asit_quality_yield a
join(
...
)c on last_day(a.month)=c.month and a.fusionid=c.fusionid
...
and team_leader not in (select name from not_teamleader)
group by month,dept
)b on last_day(a.month)=b.monthn and b.dept=c.dept
--where a.roster_month=last_day('31-JAN-24') 
--and a.ucid in (select distinct ucid from asit_roster_table where fusionid='105554' ) 
group by last_day(a.month),a.fusionid,c.ucid,a.employee_name,a.supervisor_name,c.dept,b."Out Of",c.tl_fusionid
) a 
join(
select month,fusionid,ucid,livevox_id,name,team_leader,location,dept,designation 
,tl_fusionid
